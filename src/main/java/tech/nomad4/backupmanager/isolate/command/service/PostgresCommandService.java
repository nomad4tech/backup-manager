...
import tech.nomad4.backupmanager.isolate.command.dto.CommandType;
...
/**
 * PostgreSQL-specific implementation of {@link DatabaseCommandService}.
 * <p>
 * Translates {@link CommandType#SQL} commands into {@code psql} invocations and
 * delegates execution to {@link DockerExecExecutor}. Asynchronous operations are
 * managed through {@link AsyncOperationManager}.
 * </p>
 * <p>
 * This service has no dependency on the discovery or socket modules - it works
 * purely with a {@link DockerClient} and a plain container ID string.
 * </p>
 */
@Slf4j
public class PostgresCommandService implements DatabaseCommandService {

    private final DockerExecExecutor executor;
    private final AsyncOperationManager operationManager;
    private final long defaultTimeout;

    public PostgresCommandService(DockerExecExecutor executor, AsyncOperationManager operationManager, long defaultTimeout) {
    ...
    public long getDatabaseSize(DockerClient client, String containerId, String databaseName) {
        String sql = "SELECT pg_database_size('" + databaseName.replace("'", "''") + "')";
        DatabaseCommand command = DatabaseCommand.sql(sql, "-At", "-d", "postgres").withTimeout(10L);
        CommandResult result = execute(client, containerId, command);
        if (!result.isSuccess()) {
            throw new CommandExecutionException("Failed to query PostgreSQL database size: " + result.getError());
        }
    ...
    /**
     * Wraps a SQL query in a {@code psql} invocation.
     * <p>
     * Convention for {@link CommandType#SQL} args:
     * <ul>
     *   <li>{@code args[0]} - the SQL query text</li>
     *   <li>{@code args[1..n]} - optional extra psql flags (e.g., {@code -d mydb})</li>
     * </ul>
     * Runs via {@code sh -c} so that the container's {@code $POSTGRES_USER}
    ...
    private String[] buildSqlCommand(DatabaseCommand command) {
        ...
        psqlCmd.append("psql -U \"$POSTGRES_USER\"");

        for (int i = 1; i < args.size(); i++) {
            psqlCmd.append(' ').append(shellQuote(args.get(i)));
        }

        psqlCmd.append(" -c ").append(shellQuote(sql));
        return new String[]{"sh", "-c", psqlCmd.toString()};
    }
